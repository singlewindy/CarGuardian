...
#define CNLOCALE [[NSLocale alloc] initWithLocaleIdentifier:@"zh"]
...
{    
    ...
    // The Day Before Yesterday = "前天 13:28"
    if ([self isTheDayBeforeYesterday:now]) {
        return [self formatAsTheDayBeforeYesterday];
    }
...
- (BOOL)isTheDayBeforeYesterday:(NSDate *)now
{
    return [self isSameDayAs:[now dateBySubtractingDays:2]];
}
...
// The Day Before Yesterday = "前天 13:28"
- (NSString *)formatAsTheDayBeforeYesterday
{
    //Create date formatter
    NSDateFormatter *dateFormatter = [[NSDateFormatter alloc] init];
